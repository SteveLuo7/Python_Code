...
新增功能
   ...
   mytank cant through the enemy tanks
   my tank and enemytank cant move 
   use stay()
...
class MainGame():
    ...
    def startGame(self):
        ...
        self.createWall()
        #   设置窗口标题
        pygame.display.set_caption("坦克大战1.24")
        while True:
            #   让坦克移动速度变慢
            time.sleep(0.05)
            #   设置填充颜色
            ...
            if MainGame.my_tank and MainGame.my_tank.live:
                if not MainGame.my_tank.stop:
                    ...
                    #   check myTank collide the wall or not
...
class Tank(BaseItem):
    ...
    def move(self):
        #   after move, resign the original coordinate
        self.oldLeft = self.rect.left
        self.oldTop = self.rect.top
        #   判断坦克的方向进行移动
        if self.direction == 'L':
